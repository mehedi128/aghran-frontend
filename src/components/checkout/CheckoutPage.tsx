...
import { ShieldCheck, Truck, ArrowLeft, Banknote, Lock } from 'lucide-react';
import { useStore } from '../../context/StoreContext';
import { OrderCustomerInfo } from '../../types';

export const CheckoutPage: React.FC = () => {
    const { cart, cartCount, cartSubtotal, shippingFee, discountAmount, cartTotal, deliveryZone, setDeliveryZone, placeOrder, navigateTo } = useStore();
    ...
    if (cart.length === 0) {
        return (
            <div className='max-w-xl mx-auto px-4 py-16 text-center space-y-4'>
                <h2 className='text-xl font-bold font-serif-bangla text-[#3A2A1E]'>আপনার কার্ট বর্তমানে খালি আছে</h2>
                <p className='text-xs text-[#888780]'>Please add items to your cart before proceeding to checkout.</p>
                <button onClick={() => navigateTo('collection', { category: 'all' })} className='bg-[#D85A30] text-[#FAF6EE] text-xs font-bold px-6 py-3 rounded-xl'>
                    Browse Products
                </button>
            </div>
        );
    ...
    return (
        <div className='max-w-7xl mx-auto px-4 py-8' id='checkout-page'>
            ...
            <div className='mb-8 flex items-center justify-between pb-4 border-b border-[#D85A30]/20'>
                <div>
                    <h1 className='text-2xl sm:text-3xl font-bold font-serif-bangla text-[#3A2A1E]'>Checkout (অর্ডার কনফার্মেশন)</h1>
                    <p className='text-xs text-[#888780] mt-0.5'>Fill in your delivery address to receive pure homemade products at your doorstep.</p>
                </div>
                <button onClick={() => navigateTo('cart')} className='text-xs font-bold text-[#D85A30] hover:underline flex items-center gap-1'>
                ...
                <div className='lg:col-span-7 space-y-6'>
                    ...
                    <div className='bg-[#FAF6EE] rounded-3xl border border-[#D85A30]/20 p-6 sm:p-8 shadow-sm space-y-5'>
                        <div className='flex items-center gap-2.5 pb-3 border-b border-[#D85A30]/15'>
                            <div className='w-7 h-7 rounded-full bg-[#D85A30] text-[#FAF6EE] flex items-center justify-center text-xs font-black'>1</div>
                            <h2 className='text-base sm:text-lg font-bold font-serif-bangla text-[#3A2A1E]'>Delivery Address & Contact (ডেলিভারি ঠিকানা)</h2>
                        </div>

                        {/* Delivery Zone Toggle */}
                        <div className='space-y-1.5'>
                            <label className='text-xs font-bold text-[#3A2A1E]'>Select Delivery Zone:</label>
                        ...
                        <div className='space-y-1'>
                            <label className='text-xs font-bold text-[#3A2A1E] block'>Your Full Name (আপনার নাম) *</label>
                            <input
                                type='text'
                        ...
                        {/* Phone Number */}
                        <div className='space-y-1'>
                            <label className='text-xs font-bold text-[#3A2A1E] block'>Mobile Number (মোবাইল নম্বর) *</label>
                            <div className='relative'>
                                <span className='absolute left-3.5 top-1/2 -translate-y-1/2 text-xs font-bold text-[#888780]'>+880</span>
                                <input
                                    type='tel'
                        ...
                        <div className='space-y-1'>
                            <label className='text-xs font-bold text-[#3A2A1E] block'>Full Street Address & Landmark (বাড়ি, রোড, ব্লক ও বিস্তারিত ঠিকানা) *</label>
                            <textarea
                                rows={3}
                            ...
                            />
                            {errors.streetAddress && <p className='text-[11px] text-red-500'>{errors.streetAddress}</p>}
                        </div>
                    </div>
                    ...
                    <div className='bg-[#FAF6EE] rounded-3xl border border-[#D85A30]/20 p-6 sm:p-8 shadow-sm space-y-5'>
                        <div className='space-y-3'>
                            ...
                            <div className='p-4 rounded-2xl border-2 border-[#639922] bg-[#FAEEDA] flex items-start gap-3.5 shadow-sm'>
                                <input type='radio' name='payment' value='cod' checked={true} readOnly className='accent-[#639922] mt-1' />
                                <div className='flex-1'>
                                    <div className='flex items-center gap-2 font-bold text-xs sm:text-sm text-[#3A2A1E]'>
                                        ...
                                        <span className='bg-[#639922] text-white text-[10px] font-black px-2 py-0.5 rounded-full'>১০০% নিরাপদ</span>
                                    </div>
                                    <p className='text-xs text-[#888780] mt-1 leading-relaxed'>ডেলিভারি রাইডারের কাছ থেকে পণ্য বুঝে পেয়ে দেখে মূল্য পরিশোধ করুন। অগ্রিম কোনো পেমেন্ট লাগবে না।</p>
                ...
                <div className='lg:col-span-5 space-y-6'>
                    <div className='bg-[#FAF6EE] rounded-3xl border-2 border-[#D85A30]/25 p-6 shadow-md space-y-5 sticky top-28'>
                            ...
                            {cart.map((item) => (
                                <div key={`${item.product.id}-${item.selectedVariant?.id || 'base'}`} className='flex items-center gap-3 pt-2 first:pt-0'>
                                    ...
                                    <div className='flex-1 min-w-0'>
                                        <h4 className='text-xs font-bold font-serif-bangla text-[#3A2A1E] truncate'>{item.product.nameBangla}</h4>
                        ...
                        <div className='space-y-2 text-xs text-[#3A2A1E] pt-3 border-t border-[#D85A30]/15'>
                            ...
                            <div className='flex justify-between'>
                                <span className='text-[#888780]'>Delivery Charge ({formData.deliveryZone === 'dhaka' ? 'Dhaka Metro' : 'Outside Dhaka'}):</span>
                                <span className='font-semibold'>{shippingFee === 0 ? <span className='text-[#639922] font-bold'>FREE</span> : `৳${shippingFee}`}</span>
                            </div>
